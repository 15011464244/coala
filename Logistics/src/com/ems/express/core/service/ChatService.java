...
//		Log.e("entity", entity + "");
		...
		// 派送信息推送
			...
			if ("2".equals(obj.getString("messageType"))) {
				...
				String integral = jsonObject.getString("integral");
				String invitation_mobile = jsonObject.getString("invitation_mobile");
				App.dbHelper.insertSendNoticeInvitation(App.db, "1",messageTime,invitedMobile, "6", integral,invitation_mobile);
				// 发送广播
				sendReceiver();
				boolean background = NotificationUtil
						.isBackground(getApplicationContext());
				...
				if (App.dbHelper.seleteReceivePaymentMessage(App.db, orderNum)) {
					...
					App.dbHelper.insertReceivePaymentMessage(App.db, "1", "7",messageTime,senderMobile,orderNum, price,mobNum,userCode,orgCode);
					// 发送广播
					sendReceiver();
	...
	private void querySignMessage(final String orgCode,
			...
			final String integral ,final String senderMobile) {
