...
import com.cn.net.cnpl.db.MailHandDetailHelper;
...
public class MailHandDetailDao extends MailHandDetailHelper {
	...
	public MailHandDetailDao(Context context, String name, CursorFactory factory,
			int version) {
...
//		}
//	}
	public synchronized boolean SaveMail(JSONObject dataList,long sid, String oldsid)
			throws JSONException {
		...
		if (dataList != null) {
				JSONArray objArray = new JSONArray(dataList.get("dataList").toString());
				int tempsize = objArray.length();
				for (int i = 0; i < tempsize; i++) {
					values = new ContentValues();
					values.put("mail_num", objArray.getJSONObject(i).getString("mailCode"));
					values.put("mail_type", objArray.getJSONObject(i).getString("mailSate"));
					values.put("principal", objArray.getJSONObject(i).getString("principal"));
					values.put("principal_type",
							"");//objArray.getJSONObject(i).getString("principalType"));
					values.put("abnormity_time",
							objArray.getJSONObject(i).getString("abnormityTime"));
					values.put("create_time", objArray.getJSONObject(i).getString("operationTime"));
					values.put("upload_time", objArray.getJSONObject(i).getString("uploadTime"));
					values.put("sid", sid);

					values.put("is_out", Global.MAILIN);// params.getString("is_out"));
					values.put("out_time", "");// params.getString("out_time"));
					values.put("code2d_num", "");// params.getString("code2d_num"));
					values.put("paper_num", "");// params.getString("paper_num"));
	...
	public synchronized List<Map<String, Object>> FindMail(String sid_time,String is_out,int pageNo) {
		List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
		Map<String, Object> paramsMap = null;
		Cursor cursor = null;
		String[] colums = { "mail_num", "mail_type","principal","principal_type","abnormity_time", "create_time","upload_time",
				"sid","is_out","out_time","code2d_num", "paper_num","operatorType","oldSid","signatureImg"};
		try {
			SQLiteDatabase db = getReadableDatabase();
			if(pageNo==-1){
				if("".equals(is_out)){
					cursor = db.query(TABLE_NAME, colums," sid=?  ",
							new String[] { sid_time }, null,
							null, null,null);
				}else if("".equals(sid_time)){
					cursor = db.query(TABLE_NAME, colums," is_out=? ",
							new String[] { is_out }, null,
							null, null,null);
				}else{
					cursor = db.query(TABLE_NAME, colums," sid=? and is_out=? ",
							new String[] { sid_time,is_out }, null,
							null, null,null);
				}
			}else{
				if("".equals(is_out)){
	...
	public synchronized List<Map<String, Object>> FindChange(String sid_time,boolean ss) {
		List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
		Map<String, Object> paramsMap = null;
		Cursor cursor = null;
		String[] colums = { "mail_num", "mail_type","principal","principal_type","abnormity_time", "create_time","upload_time",
		...
		try {
			SQLiteDatabase db = getReadableDatabase();
			if(ss)
				cursor = db.query(TABLE_NAME, colums," sid=? and is_out='0'",
							new String[] { sid_time }, null,
							null, null,null);
			else
				cursor = db.query(TABLE_NAME, colums," sid=? and is_out='1' and upload_time=''",
						new String[] { sid_time }, null,
						null, null,null);
			if (cursor.getCount() > 0) {
				while (cursor.moveToNext()) {
	...
	public synchronized List<Map<String, Object>> FindOff(String sid_time) {
		...
		try {
			SQLiteDatabase db = getReadableDatabase();
				cursor = db.query(TABLE_NAME, colums," sid=? and upload_time=''",
						new String[] { sid_time }, null,
						null, null,null);
			if (cursor.getCount() > 0) {
				while (cursor.moveToNext()) {
	...
	public synchronized List<Map<String, Object>> FindMail(String sid_time,String is_out) {
		List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
		Map<String, Object> paramsMap = null;
		Cursor cursor = null;
		String[] colums = { "mail_num", "mail_type","principal","principal_type","abnormity_time", "create_time","upload_time",
		...
		try {
			SQLiteDatabase db = getReadableDatabase();
			cursor = db.query(TABLE_NAME, colums," sid=? and is_out=? and upload_time='' ",
							new String[] { sid_time,is_out }, null,
							null, null,null);
			if (cursor.getCount() > 0) {
				while (cursor.moveToNext()) {
	 ...
	 * @return
	 */
	public synchronized List<Map<String, Object>> FindMailNos() {
		List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
		Cursor cursor = null;
		Map<String, Object> paramsMap = null;
		String sql=" select mail_num,operatorType,oldSid from tb_mail_hand_detail where  is_out = '0' and  sid in (select sid from tb_mail_hand  where hand_state = '2'  and is_shift_stop != '0' ) ";
		try {
			SQLiteDatabase db = getReadableDatabase();
	 ...
	 * @return
	 */
	public synchronized List<Map<String,String>> FindMailIn() {
		List<Map<String, String>> dataList = new ArrayList<Map<String, String>>();
		Cursor cursor = null;
		Map<String, String> paramsMap = null;
		String sql=" select mail_num,sid from tb_mail_hand_detail where  is_out != '1' and  sid in (select sid from tb_mail_hand where hand_type = '1' and hand_state = '2'  and is_shift_stop != '0') ";
		try {
			SQLiteDatabase db = getReadableDatabase();
	 ...
	 * @return
	 */
	public synchronized String FindMailInDlv(String mail) {
		Cursor cursor = null;
		String dataList = null;
		String sql=" select oldSid from tb_mail_hand_detail where mail_num = '" + mail+"' and is_out != '1'";
		try {
			SQLiteDatabase db = getReadableDatabase();
	...
	public synchronized List<Map<String, Object>> FindMailByUploadIN(String sid) {
		List<Map<String, Object>> dataList = new ArrayList<Map<String, Object>>();
		Map<String, Object> paramsMap = null;
		Cursor cursor = null;
		String[] colums = { "mail_num", "mail_type","principal","principal_type","abnormity_time", "create_time","upload_time",
		...
		try {
			...
			cursor = db.query(TABLE_NAME, colums," upload_time='' and sid = ? ",new String[] { sid }, null,
						null, null);
	...
	public synchronized String Findcount(String sid,String mail_type) {
		...
		try {
			...
			if("".equals(mail_type))
				...
				cursor = db.query(TABLE_NAME, colums,
						" sid=? and mail_type=?", 
						new String[] { sid,mail_type }, null, null, null);
	...
	public synchronized String FindcountMail(String sid) {
		String count = "0";
		Cursor cursor = null;
		String[] colums = { "count(1) as num" };
	...
	public synchronized String FindcountUpload(String sid) {
		String count = "0";
		Cursor cursor = null;
		String[] colums = { "count(1) as num" };
	...
	public synchronized String IsSaveMail(String is_out) {
		...
		try {
			...
			cursor = db.query(TABLE_NAME, colums," is_out=?", 
						new String[] { is_out }, null, null, "sid  desc ", null);
			if (cursor.getCount() > 0) {
				while (cursor.moveToNext()) {
					paramsMap = new LinkedHashMap<String, Object>();
					paramsMap.put("mail_num", cursor.getString(0));
	...
	public synchronized boolean ExitMail(String mail_num,String is_out) {
		...
		try {
			SQLiteDatabase db = getReadableDatabase();
				...
				cursor = db.query(TABLE_NAME, colums," mail_num=? and is_out=? ",
					new String[] { mail_num,is_out }, null,
					null, null,null);
	...
	public synchronized boolean updateMail(String mail_num,String sid_time, String is_out,String out_time,String upload_time) {
		SQLiteDatabase db = getWritableDatabase();
		if (sid_time != null) {
			...
			db.update(TABLE_NAME, contentValues,
					" sid=? and mail_num=? ", new String[] { sid_time,mail_num });
			return true;
		}
		return false;
	}
	 ...
	 * @throws JSONException
	 */
	public void UpdateMail( String sid,String downsid) throws JSONException {
	 ...
	 * @throws JSONException
	 */
	public void UpdateMailDlv(String downsid,String is_out,String mail_num) throws JSONException {
		...
		if (downsid != null && !"".equals(downsid)) {
				...
				contentValues.put("is_out", is_out);
				db.update(TABLE_NAME, contentValues,
						"oldSid=?  and mail_num =? ", new String[] {downsid,mail_num});
		}
	}
	...
	public synchronized void UpdateMail_isout(String mail,String sid) {
		...
		if (sid != null && !"".equals(sid) && mail!=null&&!"".equals(mail)) {
				...
				db.update(TABLE_NAME, contentValues,
						"sid=? and mail_num=? ", new String[] {sid,mail});
		}
	}
	...
	public synchronized void deleteMail(String mail_num,String create_time) {
		...
		if (mail_num != null) {
			db.delete(TABLE_NAME,
					"mail_num=? and create_time=? ",
					new String[] { mail_num, create_time});
		}
	}
	
	public synchronized void deleteMail(String sid) {
		SQLiteDatabase db = getWritableDatabase();
		if (sid != null) {
