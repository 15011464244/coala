...
//	private SendNoticeBean mSendNoticeBean;
	...
	Runnable runnable = new Runnable() {
		...
		public void run() {
			...
			if(messageIsSign.equals("1")){
			queryFindEmlpoyeeMessageByPhone(orgcode,username);
			}else if(messageIsSign.equals("2")){
	 ...
	 * @param username
	 */
	public void queryFindEmlpoyeeMessageByPhone(final String orgcode ,String username) {
					...
					public void onResponse(MessageCenterBean arg0) {
						...
						if (arg0 != null) {
							if ("1".equals(arg0.getResult())) {
								mDeliveryMessage = arg0.getBody().getSuccess();
	...
	//获取快递员的信息（发送邮件的临时用）
		public static void getPostmanInfo(Activity mContext,Boolean flag){
			mailSend = flag;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if(!DeviceUtil.isNetworkAvailable(this)){
			ToastUtil.show(this, "请检查您的网络是否开启");
			util.dismiss();
			return;
		}*/
		...
		coder.setOnGetGeoCodeResultListener(new OnGetGeoCoderResultListener() {
					...
					//地址输入框赋初始值
					 ...
					 if(isFirstLoc2){
						if(result.getAddressDetail() != null){
							...
							if(strpro.length() > 0){
								strpro = strpro.subSequence(0, strpro.length()-1).toString();
							}
							strcity = result.getAddressDetail().city;
							if(strcity.length() > 0){
								strcity = strcity.subSequence(0, strcity.length()-1).toString();
							}
							strcounty = result.getAddressDetail().district;
							text.setText(strpro+" "+strcity+" "+strcounty);
						}
			...
			public void onGetGeoCodeResult(GeoCodeResult result) {
				if (result != null) {
//					ToastUtil.show(getApplicationContext(), result.getAddress());
				}
			}
		});
		...
		jumpSend.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(SpfsUtil.loadPhone().equals("")){
					Intent intent1 = new Intent(BaiduMapActivity.this,LoginActivity.class);
					startActivity(intent1);
					Toast.makeText(BaiduMapActivity.this,"请先登陆", 1).show();
				}else{
					Intent intent = new Intent(BaiduMapActivity.this, SendActivity.class);
								...
								new Thread() {
									...
									public void run() {
										...
										for (ServicePoint servicePoint : servicePoints) {
											...
											LatLng ll = new LatLng(servicePoint
													.getLat(), servicePoint
													.getLon());
											addMapMark(ll);
											if (servicePoints
...
//												setLocation(ll);
							...
							@Override
							public void onErrorResponse(VolleyError arg0) {
								Toast.makeText(BaiduMapActivity.this, "数据加载失败", Toast.LENGTH_LONG).show();
				...
				new Thread() {
					...
					public void run() {
						...
						for (ServicePoint servicePoint : servicePoints) {
							...
							System.out.println("servicePoint: "
									+ servicePoint.getName());
							LatLng ll = new LatLng(servicePoint.getLatitude(),
...
//					mLat = mLat.substring(0, mLat.length());
				...
				for (Carrier carrier : carriers) {
					if (carrier.getLatitude() == l.latitude
						...
						if(mailSend){
							...
							Log.e("msgggCCC",carrier.toString() );
			...
			if ("messageCenter".equals(activity)) {
				...
				if (mDeliveryMessage.getSid() == null
						|| mDeliveryMessage.getSid().equals("")) {
					car.setSID("null");
				} else {
